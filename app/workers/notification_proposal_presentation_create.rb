class NotificationProposalPresentationCreate < NotificationSender
  ...
  def elaborate(proposal_presentation_id)
    ...
    data = {'proposal_id' => proposal.id.to_s, 'user_id' => user.id.to_s, 'title' => proposal.title}
    notification_a = Notification.create(notification_type_id: NotificationType::AUTHOR_ACCEPTED, url: url_for_proposal(proposal, proposal.group), data: data)
    ...
    data = {'proposal_id' => proposal.id.to_s, 'user_id' => user.id.to_s, 'user' => name, 'title' => proposal.title}
    notification_b = Notification.create(notification_type_id: NotificationType::NEW_AUTHORS, url: url_for_proposal(proposal, proposal.group), data: data)
