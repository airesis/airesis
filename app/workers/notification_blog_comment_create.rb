class NotificationBlogCommentCreate < NotificationSender
  ...
  def self.perform(comment_id)
    NotificationBlogCommentCreate.new.elaborate(comment_id)
  end
  ...
  def elaborate(comment_id)
    ...
    unless comment_user == post_user #don't send a notification to myself
      ...
      if another
        ...
        data = {'blog_post_id' => blog_post.id.to_s, 'blog_comment_id' => blog_comment.id.to_s, 'subject' => "[#{blog_post.title}] Nuovo commento di #{comment_user.fullname}", 'user' => comment_user.fullname, 'title' => blog_post.title, 'i18n' => 't', 'count' => 1}
