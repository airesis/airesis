class NotificationProposalCreate < NotificationSender
  ...
  def elaborate(current_user_id,proposal_id,group_id = nil,group_area_id=nil)
    ...
    if group_id
      ...
      notification_a = Notification.new(:notification_type_id => 10, :url => group_proposal_url(group,proposal, host: host), :data => data)
      notification_a.save
      ...
      notification_a = Notification.new(:notification_type_id => 3, :url => proposal_url(proposal,{subdomain: false, host: host}), :data => data)
      notification_a.save
