module GroupsHelper
  ...
  if Rails.env == 'development'
    names = Airesis::Application.routes.routes.map(&:name).select { |n| n =~ /(group_frm_|group_forum).*/ }
    names.each do |name|
  ...
  def self.init
    names = Airesis::Application.routes.routes.map(&:name).select { |n| n =~ /(group_frm_|group_forum|participation_role|group_event).*/ }
    names.each do |name|
