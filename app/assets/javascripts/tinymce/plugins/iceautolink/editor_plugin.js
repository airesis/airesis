(function () {
    tinymce.create("tinymce.plugins.IceAutolinkPlugin", {init: function (a, b) {
        var c = this;
        ...
        a.onKeyUp.add(function (ed, event) {
            if (event.keyCode == 32) {
                return c.handleSpacebar(ed)
            }
        })
    ...
    }, parseCurrentLine: function (ed, d, b, g) {
        var a, f, c, n, k, m, h, e, j;
        a = ed.selection.getRng(true).cloneRange();
        if (a.startOffset < 5) {
        ...
        if (h) {
            ...
            k = ed.selection.getBookmark();
            ed.selection.setRng(a);
            tinyMCE.execCommand("createlink", false, h[1] + h[2]);
            ed.selection.moveToBookmark(k);
            ed.nodeChanged();
            if (tinyMCE.isWebKit) {
        ...
        return{longname: "IceAutolink", author: "Rodi Alessandro", authorurl: "http://www.rodialessandro.ir", infourl: "http://wiki.moxiecode.com/index.php/TinyMCE:Plugins/autolink", version: tinymce.majorVersion + "." + tinymce.minorVersion}
    }});
    tinymce.PluginManager.add("iceautolink", tinymce.plugins.IceAutolinkPlugin)
})();