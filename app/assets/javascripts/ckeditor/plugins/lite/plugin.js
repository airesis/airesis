...
function LiteClass(ed, path) {
     ...
     */
    this.toggleTracking = function (track, bNotify) {
     ...
     */
    this.toggleShow = function (show, bNotify) {
     ...
     */
    this.acceptAll = function (options) {
     ...
     */
    this.rejectAll = function (options) {
     ...
     */
    this.countChanges = function (options) {
        return ((this._tracker && this._tracker.countChanges(options)) || 0);
    };

    this.enableAcceptReject = function (bEnable) {
     ...
     */
    this.filterIceElement = function (e) {
        if (!e) {
            return true;
        }
        try {
    ...
    this._getCleanText = function (e, textFragments, deleteClass) { // assumed never to be called with a text node
    ...
    this._onDomLoaded = function (dom) {
    ...
    this._onScriptsLoaded = function (completed, failed) {
        //console.log("ICE: scripts loaded");
        this._scriptsLoaded = true;
        this._onReady();
    };

    this._loadCSS = function (doc) {
    ...
    this._onToggleShow = function (event) {
        this.toggleShow();
    }
    ;

    this._onToggleTracking = function (event) {
        this.toggleTracking();
    }
    ;

    this._onRejectAll = function (event) {
        this.rejectAll();
    }
    ;

    this._onAcceptAll = function (event) {
        this.acceptAll();
    }
    ;

    this._onAcceptOne = function (event) {
    ...
    this._onRejectOne = function (event) {
        var node = this._tracker.currentChangeNode();
        if (node) {
    ...
    this._setButtonTitle = function (button, title) {
     ...
     */
    this._setCommandsState = function (commands, state) {
    ...
    this._onIceTextChanged = function (e) {
        this._triggerChange();
    }
    ;
    ...
    this._notifyChange = function () {
        this._changeTimeout = null;
        this._editor.fire('change');
    }
    ;

    this._commandNameToUIName = function (command) {
        return command.replace(".", "_");
    }
    ;

    this._setPluginFeatures = function (editor, props) {
...
function LiteP() {
    ...
    this.init = function (ed) {
        new LiteClass(ed, this.path);
    }
}

(function () {

    CKEDITOR.plugins.add('lite', new LiteP);
