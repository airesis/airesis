class PostPublishing < ActiveRecord::Base
  ...
  scope :viewable_by, ->(user) { joins(blog_post: {groups: :group_participations}).where("blog_posts.status = 'P' or (blog_posts.status = 'R' and group_participations.user_id = ?)", user.id) }
end
