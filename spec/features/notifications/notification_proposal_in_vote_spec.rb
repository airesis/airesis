...
describe 'notifications when the vote for the proposal starts', type: :feature do
  ...
  it "sends correctly an email to all authors and participants" do
    user1 = create(:user)
    proposal = create(:public_proposal, quorum: BestQuorum.visible.first, current_user_id: user1.id, votation: {choise: 'new', start: 10.days.from_now, end: 14.days.from_now})
    participants = []
    2.times do
      user = create(:user)
      participants << user
