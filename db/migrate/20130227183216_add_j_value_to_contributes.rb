class AddJValueToContributes < ActiveRecord::Migration
  def up
    ...
    ProposalComment.contributes.each do |proposal_comment|
      num_pos = ProposalCommentRanking.count(conditions: ["proposal_comment_id = ? AND ranking_type_id = ?", proposal_comment.id, RankingType::POSITIVE])
      num_neg = ProposalCommentRanking.count(conditions: ["proposal_comment_id = ? AND ranking_type_id = ?", proposal_comment.id, RankingType::NEGATIVE])
